...
class OrderDetail extends EntityBase {
	...
	public function LoadById($id) {
		$this->connector->CommandText = "SELECT a.*,b.bsatbesar,b.bsatkecil FROM t_ar_so_detail AS a Join m_barang AS b On a.item_code = b.bkode WHERE a.id = ?id";
	...
	public function LoadBySoId($soId, $orderBy = "a.id") {
		$this->connector->CommandText = "SELECT a.*,b.bsatbesar,b.bsatkecil FROM t_ar_so_detail AS a Join m_barang AS b On a.item_code = b.bkode WHERE a.so_id = ?soId ORDER BY $orderBy";
    ...
    public function LoadBySoNo($poNo, $orderBy = "a.id") {
        $this->connector->CommandText = "SELECT a.*,b.bsatbesar,b.bsatkecil FROM t_ar_so_detail AS a Join m_barang AS b On a.item_code = b.bkode WHERE a.so_no = ?poNo ORDER BY $orderBy";
...
"INSERT INTO t_ar_so_detail(so_id, cabang_id, so_no, item_id, item_code, item_descs, l_qty, s_qty, order_qty, send_qty, price, disc_formula, disc_amount, sub_total)
VALUES(?so_id, ?cabang_id, ?so_no, ?item_id, ?item_code, ?item_descs, ?l_qty, ?s_qty, ?order_qty, ?send_qty, ?price, ?disc_formula, ?disc_amount, ?sub_total)";
		$this->connector->AddParameter("?so_id", $this->SoId);
        $this->connector->AddParameter("?cabang_id", $this->CabangId);
        $this->connector->AddParameter("?so_no", $this->SoNo);
        ...
        $this->connector->AddParameter("?send_qty", $this->SendQty);
