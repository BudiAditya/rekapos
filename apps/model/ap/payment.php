...
class Payment extends EntityBase {
    ...
    public function LoadByEntityId($entityId) {
        $this->connector->CommandText = "SELECT a.* FROM vw_ap_payment_master AS a WHERE a.entity_id = ?entityId";
	...
	public function Insert() {
		$this->connector->CommandText = "INSERT INTO t_ap_payment_master(warkat_type_id, warkat_no, warkat_date, warkat_bank_id, warkat_descs, return_no,cabang_id, payment_no, payment_date, bank_id, creditor_id, payment_descs, payment_mode, payment_amount, allocate_amount, payment_status, createby_id, create_time, no_voucher)
VALUES(?warkat_type_id, ?warkat_no, ?warkat_date, ?warkat_bank_id, ?warkat_descs, ?return_no, ?cabang_id, ?payment_no, ?payment_date, ?bank_id, ?creditor_id, ?payment_descs, ?payment_mode, ?payment_amount, ?allocate_amount, ?payment_status, ?createby_id, NOW(), ?no_voucher)";
...
WHERE id = ?id";
    ...
    public function GetJSonUnpaidGrns($cabangId = 0,$supplierId = 0 ,$sort = 'a.grn_no',$order = 'ASC') {
        $sql = "SELECT a.id,a.grn_no,a.grn_date,a.due_date,a.balance_amount FROM vw_ap_purchase_master AS a";
