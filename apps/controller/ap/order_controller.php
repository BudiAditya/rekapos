...
class OrderController extends AppController {
    ...
    public function report(){
        ...
        if (count($this->postData) > 0) {
            ...
            $order = new Order();
            if ($sJnsLaporan == 1){
                $reports = $order->LoadPo4Reports($this->userCompanyId,$sCabangId,$sContactsId,$sSoStatus,$sStartDate,$sEndDate);
            }elseif ($sJnsLaporan == 2){
                $reports = $order->LoadPo4ReportsDetail($this->userCompanyId,$sCabangId,$sContactsId,$sSoStatus,$sStartDate,$sEndDate);
            }else{
                $reports = $order->LoadPo4ReportsRekapItem($this->userCompanyId,$sCabangId,$sContactsId,$sSoStatus,$sStartDate,$sEndDate);
            }
        }else{
            $sCabangId = 0;
            $sContactsId = 0;
