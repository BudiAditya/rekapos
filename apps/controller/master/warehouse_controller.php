...
class WarehouseController extends AppController {
	...
	public function index() {
		...
		if (!$router->IsAjaxRequest) {
			...
			$settings["from"] = "m_warehouse AS a";
	...
	public function add() {
        ...
        if (count($this->postData) > 0) {
            $warehouse->CabangId = $this->userCabangId;
        ...
        if (count($this->postData) > 0) {
            ...
            if ($this->ValidateData($warehouse)) {
                ...
                if ($rs == 1) {
                    ...
                    $this->persistence->SaveState("info", sprintf("Perubahan data satuan: %s (%s) sudah berhasil disimpan", $warehouse->WhName, $warehouse->WhCode));
                    redirect_url("master.warehouse");
                } else {
