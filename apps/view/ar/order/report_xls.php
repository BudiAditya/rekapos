...
$sheet->setTitle("Rekapitulasi Order Penjualan");
//helper for styling
...
$row++;
if ($JnsLaporan < 3) {
    $sheet->setCellValue("A$row","REKAPITULASI SALES ORDER");
    $row++;
    ...
    if ($JnsLaporan == 2) {
        ...
        $sheet->setCellValue("L$row", 'Terkirim');
    ...
    if ($Reports != null){
        ...
        while ($rpt = $Reports->FetchAssoc()) {
            ...
            if (!$sma) {
                $csi = $rpt["customer_name"].' ('.$rpt["customer_code"].')';
                $sheet->setCellValue("A$row",$nmr);
                $sheet->getStyle("A$row")->applyFromArray($center);
                $sheet->setCellValue("B$row",$rpt["cabang_code"]);
                $sheet->setCellValue("C$row",date('d-m-Y',strtotime($rpt["so_date"])));
                $sheet->setCellValue("D$row",$rpt["so_no"]);
                $sheet->setCellValue("E$row",$csi);
                $sheet->setCellValue("F$row",$rpt["so_descs"]);
            ...
            if ($JnsLaporan == 2) {
                ...
                $sheet->setCellValue("L$row", $rpt['send_qty']);
    ...
    $sheet->setCellValue("A$row", "REKAPITULASI ITEM SALES ORDER");
    $row++;
    $sheet->setCellValue("A$row", "Dari Tgl. " . date('d-m-Y', $StartDate) . " - " . date('d-m-Y', $EndDate));
    $row++;
    $sheet->setCellValue("A$row", "No.");
    ...
    $sheet->setCellValue("G$row", "Terkirim");
    ...
    if ($Reports != null) {
        while ($rpt = $Reports->FetchAssoc()) {
            ...
            $sheet->setCellValue("G$row", $rpt['sum_sendqty']);
