...
$pdf->SetX(-80,true);
$pdf->Cell(20, 5, "Dari Gudang", 0, 0, "L");
$pdf->Cell(4, 5, ":", 0, 0, "L");
$pdf->Cell(4, 5, $trx->WhCode, 0, 0, "L");
$pdf->Ln(5);
$pdf->SetFont("helvetica", "", 10);
...
$pdf->SetX(-80,true);
$pdf->Cell(20, 5, "Ke Gudang", 0, 0, "L");
$pdf->Cell(4, 5, ":", 0, 0, "L");
$pdf->Cell(4, 5, $trx->ToWhCode, 0, 0, "L");
$pdf->Ln(2);
$x = 6;
$y = $pdf->GetY();
//garis datar header
...
foreach($trx->Details as $idx => $detail) {
	$pdf->Ln(5);
	$pdf->Cell(22,5,$detail->Qty.' '.strtolower(left($detail->SatKecil,3)).' ',0,0,"R");
	if ($detail->ItemCode == $detail->BarCode) {
        $pdf->Cell(100, 5, $detail->ItemCode . ' - ' . $detail->ItemDescs, 0, 0, "L");
    }else{
        $pdf->Cell(100,5,$detail->ItemCode.' - '.$detail->ItemDescs.' ['.$detail->BarCode.']',0,0,"L");
    }
